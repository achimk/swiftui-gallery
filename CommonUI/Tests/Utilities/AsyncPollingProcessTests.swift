...
class AsyncPollingProcessTests: XCTestCase {
    enum Const {
    ...
    func test_pollOneTimeAndStop_shouldReceiveEventsInCorrectOrder() {
        let components = makeTestComponents(operation: { @MainActor [weak self] in
            await self?.handlePolling()
        })
...
extension AsyncPollingProcessTests {
    private enum Event {
        case pollingStarted
    ...
    @MainActor
    private func handlePolling() async {
